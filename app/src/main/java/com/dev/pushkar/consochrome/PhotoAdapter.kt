 ...
 */
class PhotoAdapter(private val mDataSet: ArrayList<Update>, var mContext: Context) : RecyclerView.Adapter<PhotoAdapter.PhotoViewHolder>() {
    private val mFoldStates = HashMap<Int, Boolean>()
    ...
    override fun onBindViewHolder(holder: PhotoViewHolder, position: Int) {
        if (mDataSet[position].sel) {
            ...
            // Bind data
            Picasso.with(holder.mFoldableLayout.context).load(path).into(holder.mImageViewCover)
            Picasso.with(holder.mFoldableLayout.context).load(path).into(holder.mImageViewDetail)
    ...
    protected class PhotoViewHolder(var mFoldableLayout: FoldableLayout) : RecyclerView.ViewHolder(mFoldableLayout) {
